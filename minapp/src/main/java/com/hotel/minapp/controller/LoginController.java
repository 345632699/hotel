...
//      if (!wxService.getUserService().checkUserInfo(sessionKey, loginQuery.getRawData(), loginQuery.getSignature())) {
//        throw new BaseException(500, "user check failed");
//      }
      stringRedisTemplate.opsForValue().set(loginQuery.getCode(), sessionKey, 60 * 60 * 24 * 7, TimeUnit.SECONDS);

      // 解密用户信息
      WxMaUserInfo userInfo = wxService.getUserService().getUserInfo(sessionKey, loginQuery.getEncryptedData(), loginQuery.getIv());
      LOGGER.info(userInfo.toString());
      List<WxConsumer> wxUser = consumerService.getConsumerByOpenId(userInfo.getOpenId());
      if (wxUser.size() <= 0) {
        consumerService.createConsumer(userInfo);
      } else {
      ...
      String sign = JWTUtil.sign(userInfo.getOpenId());
      map.put("token", sign);
